...
public class MatchPlayer
{
	...
	private ArrayList<PokerBot> bots;				// list of bots seated at the table
	...
	private long[] botTimeBanks;					// amount of time each bot has in its time bank to make moves
	...
	*/
	...
	/**
	 * Set the time bank values of the bots, the order in which the bot list was given is used to assign the time bank
	 * values. Should be called before the first call to the run(...) method. Without a call to this method all bots
	 * with start with a default time bank.
	 * @param timeBanks : long array of time banks in milliseconds.
	 */
	public void setTimeBanks(long[] timeBanks)
	{
		if(timeBanks.length == numberOfBots)
			for(int i = 0; i < numberOfBots; i++)
				botTimeBanks[i] = timeBanks[i];
		else
			System.err.println("Failed setting the time banks because the list of stacks is not of length " +
					numberOfBots);
	}
	...
	public void finishSetup(boolean sendAllMatchInfo)
	{
		...
		handHistory += "Settings gameType " + gameCode;
		
		// store extra table information in case of a tournament
		if(isTournament)
		{
	...
	/**
	 * Starts a match. Plays until a certain number of hands passed and/or until a certain number of players is left.
	 * The function returns the results of the match as an arraylist of strings.
	 * @param maxHands : the maximal number of hands to play. If value is <= 0 then it assumes no maximum for the
	 * tournament mode and for cash game mode it will play 1000 hands.
	 * @param maxPlayersLeft : the maximal number of players that should be left before quitting; this value only
	 * affects tournament mode since in cash mode no players drop out.
	 */
	public ArrayList<String> run(int maxHands, int maxPlayersLeft)
	{			
		// in case the bots are not informed about the match settings with a manual call, send all information now
		finishSetup(true);
		
		// interpret the input values correctly, based on whether this match is a tournament or not
		if(isTournament)
		{
			// regard negative hand number value as unrestricted, else correct for amount of hands already played
			if(maxHands <= 0)
		...
		{
			// regard negative hand number value as a 1000 hands, else correct for amount of hands already played
			if(maxHands <= 0)
				maxHands = handNumber + 1000;
			else
				maxHands += handNumber;
		}
		
		// continue playing hands until the stopping criteria is met
		while((isTournament && handNumber < maxHands && numberOfRemainingBots(0) > maxPlayersLeft) ||
			  (!isTournament && handNumber < maxHands))
		{
			// in a tournament, increase the blinds after every fixed amount of hands
		...
		// for a tournament, return the finish positions of the bots and the amount of chips of the remaining bots
		if(isTournament)
		{		
			for(int i = 0; i < numberOfBots; i++)
			{
				...
				{
					finalResults.add(bots.get(i).getName() + " finished " + finishPosition[i]);
					if(finishPosition[i] == 1) 
						this.winner = bots.get(i);
				}
				else
					finalResults.add(bots.get(i).getName() + " stack " + botStacks[i]);				
			}
		}
		// for cash game mode, return the summed gain/loss per bot over the whole match
		else
		{
			for(int i = 0; i < numberOfBots; i++)
			{
				finalResults.add(bots.get(i).getName() + " gainLoss " + botGainLoss[i]);
			}
		}
		return finalResults;
	}
	...
	/**
	 * Returns the current time banks of the bots, in the same order as the bot list that was given initially.
	 */
	public long[] getCurrentTimeBanks()
	{
		return botTimeBanks;
	}
	...
	*/
	 ...
	 */
	private int numberOfRemainingBots(int type)
	{
	...
	*/
	...
	{			
		...
		handHistory += String.format("\nMatch dealerButton %s", bots.get(buttonSeat).getName());
		for(int i = 0; i < numberOfBots; i++)
			if(botStacks[i] > 0)
	...
	private void askBotAction()
	{
		if(botStacks[activeSeat] > 0 && isInvolvedInHand[activeSeat])
		{
			...
			// get the next move from the current bot and time how long it took to get a reaction
			long startTime = System.currentTimeMillis();
			PokerMove nextMove = (new BotAction()).getMove(bots.get(activeSeat).getBot(), botCodeNames[activeSeat],
					botTimeBanks[activeSeat]);
			...
			if(nextMove == null)
			{
				...
				outputErrorToBot(activeSeat, "Your bot did not act in time, action set to 'check'");
			}					
			...
			if(botAction.equals(originalAction) && botActionAmount == originalActionAmount) {
				handHistory += String.format("\n%s %s %d %d", bots.get(activeSeat).getName(), botAction,
					botBetsThisRound[activeSeat], extraInfo);
			}
			else {
				handHistory += String.format("\n%s %s %d %d %s %d %s", bots.get(activeSeat).getName(), botAction,
	...
	private void dealHandCards()
	{
		for(int i = 0; i < numberOfBots; i++)
		{
			...
			if(isInvolvedInHand[index])
			{
				...
				handHistory += String.format("\n%s hand %s", bots.get(index).getName(), botHands[index].toString());
			}
		}
	...
	*/
	...
	{	
		if(!noSmallBlindPayed)
		{
			...
			handHistory += String.format("\n%s post %s", bots.get(smallBlindSeat).getName(),
		...
		handHistory += String.format("\n%s post %s", bots.get(bigBlindSeat).getName(), botBetsThisRound[bigBlindSeat]);
	...
	*/
	private int placeBet(int size, int botIndex)
	{
		...
		pot.addBet(bots.get(botIndex), size, round);
		return size;
	}
	...
	private void updateBotOdds()
	{
		...
		for(int i = 0; i < numberOfBots; i++)
		{
			...
			handHistory += String.format("\n%s odds %s%%", bots.get(i).getName(), percentage);
		}
	}
	...
	{	
		...
		{			
			...
			for(int i = 0; i < numberOfBots; i++)
				if(handStrengths[i] >= 0) {
					botHandStrengths.put(bots.get(i), handStrengths[i]);
		...
		else
		{
			...
			{
				if(isInvolvedInHand[i])
					botHandStrengths.put(bots.get(i), 1);
			}
		}
	...
	{
		MatchInfo info = new MatchInfo(type, bots, botCodeNames, isTournament, TIMEBANK_MAX, TIME_PER_MOVE,
	...
	private void sendHandInfo(HandInfoType type)
	{
		HandInfo info = new HandInfo(type, isTournament, handNumber, numberOfPlayersInTournament,
									 bots, botCodeNames, botStacks, sizeBB, sizeSB, buttonSeat,
	...
	private void sendPreMoveInfo(int amountToCall)
	{
		...
		if(botStacks[activeSeat] < amountToCall)
		{
			...
			maxPotSizeToWin = pot.getMaxPotToWin(bots.get(activeSeat), amountToCall);
		}
		else
	...
	{
		HandResultInfo resultInfo = new HandResultInfo(bots, botCodeNames, potDivision);
		if(showdown)
		{
			for(int i = 0; i < numberOfBots; i++)
				if(isInvolvedInHand[i])
	...
	{
		IORobot ioRobot = (IORobot) bots.get(seat).getBot();
 		ioRobot.addToDump("Engine says: \"" + output + "\"\n");
	}
...
//		System.out.println(handHistory);
	...
	public PokerBot winningPokerBot() 
	{
		return this.winner;
	}
